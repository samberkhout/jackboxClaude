...
export default function ChampdUpInput({ playerId }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (!name.trim()) {
      setError('Please name your champion');
      return;
    }

    const drawing = canvasRef.current.toDataURL();

    submitInput({ drawing, name: name.trim() }, (response) => {
      if (response.success) {
        setSubmitted(true);
      } else {
    ...
    return (
      <div className="relative overflow-hidden">
        ...
        <div className="relative bg-gradient-to-br from-gray-900 to-gray-800 rounded-3xl p-12 text-center shadow-2xl border-4 border-green-500/50">
          ...
          <p className="text-2xl text-gray-300 font-semibold">Waiting for challengers...</p>
    ...
    <div className="relative overflow-hidden">
      ...
      <div className="relative bg-gradient-to-br from-gray-900 to-gray-800 rounded-3xl p-8 shadow-2xl border-4 border-emerald-500/50">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-lg">Draw an epic champion!</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="transform transition-all duration-300 hover:scale-[1.02]">
            <div className="bg-gradient-to-r from-emerald-600/30 to-green-600/30 rounded-2xl p-6 border-2 border-emerald-500/50 shadow-lg">
              <label className="block text-xl font-bold text-white mb-4">🏆 Champion Name:</label>
              <input
                type="text"
                value={name}
                onChange={(e) => setName(e.target.value)}
                placeholder="Name your legendary champion..."
                className="w-full px-5 py-4 rounded-xl bg-gray-900/80 border-2 border-emerald-400/50 text-white placeholder-gray-500 text-lg font-medium focus:outline-none focus:border-teal-400 focus:ring-4 focus:ring-teal-400/30 transition-all"
                maxLength={30}
                required
              />
              <div className="flex justify-between items-center mt-3">
                <div className="text-sm text-gray-400 font-medium">
                  {name.length > 0 ? '💪 Epic name!' : '✍️ Start typing...'}
                </div>
                <div className={`text-sm font-bold ${name.length > 25 ? 'text-red-400' : 'text-emerald-400'}`}>
                  {name.length}/30
                </div>
              </div>
            </div>
          ...
          <div className="transform transition-all duration-300 hover:scale-[1.02]">
            <div className="bg-gradient-to-r from-green-600/30 to-teal-600/30 rounded-2xl p-6 border-2 border-green-500/50 shadow-lg">
              <label className="block text-xl font-bold text-white mb-4">🎨 Draw your champion:</label>
              <canvas
                ref={canvasRef}
                width={400}
                height={300}
          ...
          <button
            type="submit"
            className="w-full py-6 px-8 bg-gradient-to-r from-emerald-500 via-green-500 to-teal-500 hover:from-emerald-600 hover:via-green-600 hover:to-teal-600 text-white text-2xl font-black rounded-2xl shadow-2xl transform transition-all duration-300 hover:scale-[1.05] hover:shadow-teal-500/50 active:scale-95 border-4 border-emerald-400/50"
          >
            🚀 SUBMIT CHAMPION
          </button>
        </form>
